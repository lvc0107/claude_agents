...
#   - ch_link_payer_mock_server (pretenders, port 8000)
#   - MockServer-FtpScheduler (wiremock, port 8080)
docker ps --format "{{.Names}}" | grep -E "localdb|ch_link_payer_mock_server|MockServer-FtpScheduler"
```

If any container is **not running**, restart them:
```bash
docker restart ch_link_payer_mock_server; docker restart localdb; docker ps
```
