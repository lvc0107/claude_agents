...
```
 ECH- <ticket_id>_Description-separated-by-dashes
```

Examples:
- ` ECH- 123456_Add-user-authentication`

Rules: only alphanumeric and hyphens after the first underscore. No underscores in the description part.
Pattern enforced by pre-commit hook: `^ ECH- [0-9]+_[A-Za-z0-9-]+$`

## Instructions

### 2.1 — Discard local changes
```bash
git stash save "PREVIOUS changes to <ticket_id>"
```
...
✅ Active branch:  ECH- <ticket_id>_<description-with-dashes>
```
